...
class H5PVol:
    ...
    def H5VL_python_dataset_open(self, obj_id, loc_params, name, dapl_id, dxpl_id, req):
        ...
        try:
            # retrieve container name based on obj_id
    ...
    def H5VL_python_file_close(self, file_id, dxpl_id, req):
        try:
            if (len(self.obj_list) == 0):
                ...
                return 1
            # retrive the file handle/obj based on id 'file_id'
            ...
            if file_obj == None:
                ...
                # reset index to 0, (1 for now, due to bug of PyLong_AsVoidPtr can not take 0)
                self.obj_curid = 1
                return 1
        except Exception as e:
    ...
    def H5VL_python_dataset_close(self, dset_id, dxpl_id, req):
        try:
            if (len(self.obj_list) == 0):
                ...
                return 1
            # retrive the file handle/obj based on id 'dataset_id'
    ...
    def H5VL_python_group_create(self, obj_id, loc_params, name, gcpl_id, gapl_id, dxpl_id, req):
        ...
        try:
            ...
            try:
                ...
                swift_object_create(container=grp_parent_obj,
                                    sciobj_name=empty_sciobj_name)
                self.obj_list[curid + 1] = grp_parent_obj + \
                    '\\' + empty_sciobj_name
    ...
    def H5VL_python_dataset_create(self, obj_id, loc_params, name, dcpl_id, dapl_id, dxpl_id, req, ndims, pytype, dims,
        ...
        try:
            ...
            try:
                ...
                swift_object_create(
                    container=dst_container_name, sciobj_name=dst_obj_name)
                sci_obj_meta = {}
                sci_obj_meta['type'] = str(self.dt_types[pytype])
                sci_obj_meta['dims'] = numpy.array_str(dims)
                sci_obj_meta['ndim'] = str(ndims)
                r1 = swift_metadata_create(container=dst_container_name, sciobj_name=dst_obj_name,
                                           sciobj_metadata=sci_obj_meta)
    ...
    def H5VL_python_dataset_write(self, obj_id, mem_type_id, mem_space_id, file_space_id, plist_id, buf, req):
        ...
        try:
            ...
            try:
                metadata = swift_metadata_get(
                    container=dst_container_name, sciobj_name=dst_object_name)
                if (req >= 0):
                    dst_object_name = dst_object_name + '_' + str(
                        req)  # data of this hyperslab block, ended with start offset
                if (req == -2):
                    dst_object_name = dst_object_name + '_' + \
                        'meta'  # meta of this hyperslab block
                if (req == -3):
                    dst_object_name = dst_object_name + '_' + 'simple'  # no hyperslab enabled
                swift_object_create(
                    container=dst_container_name, sciobj_name=dst_object_name, sciobj_source=buf)
                sci_obj_meta = {}
    ...
    def H5VL_python_group_close(self, grp_id, dxpl_id, req):
        try:
            if (len(self.obj_list) == 0):
                ...
                return 1
            # retrive the group handle/obj based on id 'grp_id'
    ...
    def H5VL_python_dstobj_scan(self, obj_id, global_meta, meta_offlen, req):
        ...
        con_data = self.dst_oneshot_io(
            object_selected, obj_id, dst_object_name)
        return con_data
    ...
    def Object_Binary_Search(self, object_mappings, meta_offlen):
        ...
        for imeta in meta_offlen:
            object_selected[imeta[0]] = self.obj_binary_search(
                object_mappings, l, r, imeta)
    ...
    def obj_following(self, objm, found_index, imeta):
        ...
        objm_rich = numpy.append(objm[found_index], max(
            imeta_offset_min, objm[found_index][1]))
        objm_rich = numpy.append(objm_rich, min(
            imeta_offset_max, objm[found_index][1] + objm[found_index][2] - 1))
        ...
        while (left_start >= 0):
            if (self.obj_overlap(objm[left_start], imeta) == 0):
                objm_rich = numpy.append(objm[left_start], min(
                    imeta_offset_min, objm[left_start][1]))
                objm_rich = numpy.append(objm_rich,
                                         min(imeta_offset_max, objm[left_start][1] + objm[left_start][2] - 1))
        ...
        while (right_start < len(objm)):
            if (self.obj_overlap(objm[right_start], imeta) == 0):
                objm_rich = numpy.append(objm[right_start], min(
                    imeta_offset_min, objm[right_start][1]))
                objm_rich = numpy.append(objm_rich,
                                         min(imeta_offset_max, objm[right_start][1] + objm[right_start][2] - 1))
                result.append(objm_rich)
                right_start += 1
            else:
                break
